...
def _refresh_loop(interval: int) -> None:
    ...
    while True:
        ...
        for manifest in registry.list():
            primary = next(
                (location.uri for location in manifest.locations if location.role == "primary"),
                "",
            )
            artifacts.append({
        ...
        with _CACHE_LOCK:
            _CACHE_STATE["artifacts"] = artifacts
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802 (stdlib API)
        ...
        try:
            ...
            if parsed_path.path == "/api/artifacts.json":
                body = json.dumps(state.get("artifacts", []), default=str).encode()
                ...
                self.wfile.write(body)
                return
            if parsed_path.path == "/api/artifact.json":
                values = parse_qs(parsed_path.query)
                ref = (values.get("ref") or [""])[0]
                if not ref:
                    self.send_response(400)
                    self.end_headers()
                    return
                registry = ArtifactRegistry(JobStorage(BUCKET))
                manifest = registry.resolve_manifest(ref)
                value = manifest.to_dict()
                value["requested_ref"] = ref
                value["aliases"] = registry.aliases_for(manifest.ref)
