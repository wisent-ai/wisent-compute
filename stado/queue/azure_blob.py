...
class AzureBlobBackend:
    ...
    def download_text_versioned(self, path: str) -> tuple[str, str] | None:
        """Read text with the ETag that identifies the exact downloaded value."""
        ...
        for attempt in range(3):
            try:
                ...
                return stream.readall(), str(props.etag)
            except ResourceModifiedError:
                if attempt == 2:
                    raise
            except ResourceNotFoundError:
                return None
        raise RuntimeError("unreachable versioned Azure Blob read")
    ...
    def compare_and_swap_text(
        ...
        """Replace a text blob only when its current ETag matches."""
        from azure.core import MatchConditions
        client = self._container.get_blob_client(path)
        ...
        written_etag = response.get("etag") if hasattr(response, "get") else None
        if not written_etag:
            raise RuntimeError("Azure conditional write did not return its ETag")
        return str(written_etag)
    ...
    def download_to_filename(self, path: str, filename: str) -> bool:
        from azure.core.exceptions import ResourceNotFoundError
        try:
            stream = self._container.download_blob(path)
    ...
    def upload_text_if_absent(self, path: str, content: str) -> bool:
        from azure.core.exceptions import ResourceExistsError
        try:
            self._container.upload_blob(name=path, data=content, overwrite=False)
            return True
    ...
    def upload_file_if_absent(self, path: str, filename: str) -> bool:
        ...
        try:
            with open(filename, "rb") as source:
                self._container.upload_blob(name=path, data=source, overwrite=False)
            return True
        except ResourceExistsError:
            return False
