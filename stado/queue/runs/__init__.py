...
RUN_PREFIX = "runs"
TERMINAL_PREFIXES = ("completed", "uploaded", "failed", "cancelled")
ALL_PREFIXES = ("queue", "running", *TERMINAL_PREFIXES)
...
def run_status(store, run_id) -> dict | None:
    ...
    for jid in manifest["job_ids"]:
        ...
        if st is None:
            ...
            counts[st] += 1
    terminal = sum(counts[prefix] for prefix in TERMINAL_PREFIXES)
