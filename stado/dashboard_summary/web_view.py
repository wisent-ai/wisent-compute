...
def render_html(state: dict[str, Any], cleanup: dict[str, Any], refresh: int) -> str:
    ...
    for artifact in artifacts:
        ...
        raw_coverage = (
            f"{summary.get('raw_leaves_complete', '?')}/"
            f"{summary.get('raw_leaves_expected', '?')}"
        )
        aggregated_coverage = (
            f"{summary.get('aggregated_leaves_complete', '?')}/"
            f"{summary.get('aggregated_leaves_expected', '?')}"
        )
        detail_url = "/api/artifact.json?ref=" + quote(str(artifact.get("ref") or ""), safe="")
        artifact_rows.append(
            "<tr>"
            f'<td><a href="{_e(detail_url)}"><code>{_e(artifact.get("ref") or "")}</code></a>'
            f'<br><span class="muted">{_e(artifact.get("title") or "")}</span></td>'
            f"<td>{_e(', '.join(artifact.get('aliases') or []) or '-')}</td>"
            f"<td>{_e(artifact.get('verification') or '-')}</td>"
            f"<td>raw {_e(raw_coverage)}<br>aggregated {_e(aggregated_coverage)}</td>"
            f"<td>{_e(artifact.get('run_id') or '-')}</td>"
            f"<td><code>{_e(artifact.get('primary_uri') or '')}</code></td>"
            "</tr>"
        )
...
<h2>artifacts</h2><table><tr><th>immutable ref</th><th>aliases</th><th>verification</th><th>coverage</th><th>producer run</th><th>primary location</th></tr>{''.join(artifact_rows) or '<tr><td colspan="6" class="muted">no artifacts</td></tr>'}</table><div class="muted">JSON: <a href="/api/artifacts.json">/api/artifacts.json</a></div>
