...
_wc_ship_log() {
  ...
  gsutil -q cp /var/log/wisent-agent.log \
    "gs://${WC_BUCKET}/agent_logs/${_WC_INST}.log" 2>/dev/null \
  || gcloud storage cp /var/log/wisent-agent.log \
    "gs://${WC_BUCKET}/agent_logs/${_WC_INST}.log" 2>/dev/null || true
}
...
if [ ! -x /opt/wisent-agent/.venv/bin/wc ]; then
    ...
    pip install --upgrade stado wisent wisent-tools wisent-extractors wisent-evaluators
