...
def start_slot(store: JobStorage, job, hostname: str, log_fn,
    ...
    artifact_inputs_json = json.dumps(
        getattr(job, "resolved_input_artifacts", {}) or {},
        sort_keys=True,
        separators=(",", ":"),
    )
    artifact_inputs_file = f"{work_dir}/artifacts.json"
    Path(artifact_inputs_file).write_text(artifact_inputs_json)
    ...
    job_env = {
        **os.environ,
        ...
        "WC_ARTIFACT_INPUTS_JSON": artifact_inputs_json,
        "WC_ARTIFACT_INPUTS_FILE": artifact_inputs_file,
    }
