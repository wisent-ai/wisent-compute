...
def _mount_device(mnt: str) -> str | None:
    """Backing device for a mount point (last match in /proc/mounts wins)."""
    dev = None
    try:
        with open("/proc/mounts") as f:
            for line in f:
...
def _is_rotational(mnt: str) -> bool:
    """True if the mount's backing block device is rotational (HDD).

    Unknown resolves to True so an unidentifiable device is treated as an HDD
    and never wins over a confirmed SSD. Multi-GB shard staging is
    write-throughput bound, so an SSD is strongly preferable to a larger HDD.
    """
    dev = _mount_device(mnt)
    if not dev or not dev.startswith("/dev/"):
        return True
    try:
        name = os.path.basename(os.path.realpath(dev))
        base = name
        if not os.path.exists(f"/sys/block/{base}"):
            m = re.match(r"(nvme\d+n\d+)p\d+$", name)
            base = m.group(1) if m else re.sub(r"\d+$", "", name)
        with open(f"/sys/block/{base}/queue/rotational") as fh:
...
def setup_agent_staging(log_fn) -> str | None:
    ...
    for mnt in _candidate_mounts():
        ...
        if free <= tmp_free:
            continue
        rotational = _is_rotational(mnt)
        # Rank: prefer non-rotational (SSD/NVMe) over rotational (HDD);
        # break ties by free space. Shard staging is write-throughput bound,
        # so a smaller SSD beats a larger HDD.
        rank = (not rotational, free)
        if best is None or rank > (not best[2], best[1]):
            best = (target, free, rotational)
    if not best:
        log_fn(
