...
# (`from stado.models import GPU_SIZING`, etc.) keep working.
...
class Job:
    ...
    # Provider-neutral placement and execution requirements. Empty values mean
    # unspecified so existing GPU jobs keep their current routing semantics.
    executor: str = "stado-agent"
    platform_os: str = ""
    architecture: str = ""
    cpu_cores: int = int()
    memory_gb: int = int()
    disk_gb: int = int()
    region: str = ""
    # Structured provider execution. box-prompt uses prompt fields; command
    # executors use the existing command/repo/pre_command/verify_command fields.
    box_ttl_seconds: int = int()
    prompt: str = ""
    prompt_provider: str = ""
    prompt_model: str = ""
    prompt_reasoning_effort: str = ""
    # Named, reproducible artifact inputs. input_artifacts preserves the
    # operator's requested refs; resolved_input_artifacts pins each alias to
    # the immutable ref and primary URI at submit time.
    input_artifacts: dict = field(default_factory=dict)
    resolved_input_artifacts: dict = field(default_factory=dict)
