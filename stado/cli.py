...
main.add_command(machine)


def _resolve_input_artifacts(values):
    ...
    for value in values:
        if "=" not in value:
            raise click.ClickException(
                f"--input-artifact must be NAME=REF: {value!r}"
            )
        name, ref = value.split("=", 1)
        if not re.fullmatch(r"[A-Za-z][A-Za-z0-9_-]{0,63}", name):
            raise click.ClickException(
                f"artifact input name is unsafe: {name!r}"
            )
        if name in requested:
            raise click.ClickException(f"duplicate artifact input name: {name}")
        manifest = _artifact_call(lambda ref=ref: registry.resolve_manifest(ref))
        primary = next(
            (location for location in manifest.locations if location.role == "primary"),
            None,
        )
        if primary is None:
            raise click.ClickException(f"artifact has no primary location: {manifest.ref}")
        requested[name] = ref
        resolved[name] = {
            "ref": str(manifest.ref),
            "uri": primary.uri,
            "manifest_sha256": manifest.verification.manifest_sha256,
        }
...
@click.option(
    ...
    "input_artifacts",
    multiple=True,
    help="Pinned artifact input as NAME=TYPE/NAMESPACE/NAME@VERSION_OR_ALIAS.",
)
...
def submit(command, provider, batch_file, spot, max_cost_per_hour, any_provider, priority,
    ...
    requested_artifacts, resolved_artifacts = _resolve_input_artifacts(input_artifacts)
...
def quota_request_all(new_limit, providers_arg, regions_arg, justification,
    ...
    for p in providers:
        if p == "gcp":
            ...
            # default to stado.config.REGIONS — that's the
...
def _artifact_labels(values):
    labels = {}
    for value in values:
        if "=" not in value:
            raise click.ClickException(f"label must be KEY=VALUE: {value!r}")
        key, item = value.split("=", 1)
        if not key:
            raise click.ClickException("label key cannot be empty")
        labels[key] = item
    return labels
...
@artifact.group("import")
def artifact_import():
    """Build manifests from supported external artifact formats."""


@artifact_import.command("activations")
@click.option("--repo", required=True, help="Hugging Face dataset, e.g. wisent-ai/activations.")
@click.option("--revision", required=True, help="Immutable Hugging Face commit SHA.")
@click.option(
    "--desired-state-dir",
    required=True,
...
def artifact_import_activations(
    repo, revision, desired_state_dir, run_id, job_ids, version, aliases, full, as_json
):
    """Verify and publish a pinned activation dataset revision."""
    from .artifacts import ArtifactRegistry
    from .artifacts.adapters.activations import build_activation_manifest
    ...
    registry = ArtifactRegistry()
    published = _artifact_call(
        lambda: registry.publish(manifest, verify=True, full=full)
    )
    alias_refs = [
        str(_artifact_call(lambda alias=alias: registry.set_alias(published.ref, alias)))
        for alias in aliases
    ]
    if as_json:
        value = published.to_dict()
        value["aliases_created"] = alias_refs
        click.echo(json.dumps(value, indent=2, sort_keys=True))
    else:
        click.echo(str(published.ref))
        for alias_ref in alias_refs:
            click.echo(f"{alias_ref} -> {published.ref}")


@artifact.command("list")
@click.option("--type", "type_name", default="")
@click.option("--namespace", default="")
@click.option("--name", default="")
@click.option("--label", "labels", multiple=True, help="Filter by KEY=VALUE.")
@click.option("--json", "as_json", is_flag=True)
def artifact_list(type_name, namespace, name, labels, as_json):
    """List registered artifact versions."""
    from .artifacts import ArtifactRegistry

    registry = ArtifactRegistry()
    manifests = _artifact_call(
        lambda: registry.list(
            type_name=type_name,
            namespace=namespace,
            name=name,
            labels=_artifact_labels(labels),
        )
    )
    if as_json:
        click.echo(json.dumps([item.to_dict() for item in manifests], indent=2, sort_keys=True))
        return
    if not manifests:
        click.echo("(no artifacts found)")
        return
    click.echo(f"{'REF':<76} {'CREATED':<20} {'VERIFY':<8} ALIASES")
    for manifest in manifests:
        aliases = ",".join(registry.aliases_for(manifest.ref)) or "-"
        click.echo(
            f"{str(manifest.ref)[:75]:<76} "
            f"{manifest.created_at[:19]:<20} "
            f"{manifest.verification.result or '-':<8} {aliases}"
        )
...
@click.argument("ref")
@click.option("--json", "as_json", is_flag=True)
def artifact_show(ref, as_json):
    """Show one version or resolve an alias."""
    from .artifacts import ArtifactRegistry

    registry = ArtifactRegistry()
    manifest = _artifact_call(lambda: registry.resolve_manifest(ref))
    aliases = registry.aliases_for(manifest.ref)
    if as_json:
        value = manifest.to_dict()
        value["aliases"] = aliases
        value["requested_ref"] = ref
        click.echo(json.dumps(value, indent=2, sort_keys=True))
        return
    click.echo(f"Artifact:     {manifest.ref.coordinate()}")
    click.echo(f"Version:      {manifest.ref.version}")
    click.echo(f"Aliases:      {', '.join(aliases) or '-'}")
    click.echo(f"Title:        {manifest.title}")
    click.echo(f"Verification: {manifest.verification.result or '-'}")
    for location in manifest.locations:
        click.echo(f"Location:     [{location.role}] {location.uri}")
    if manifest.producer.run_id:
        click.echo(f"Run:          {manifest.producer.run_id}")
    if manifest.summary:
        click.echo("Summary:")
        click.echo(json.dumps(manifest.summary, indent=2, sort_keys=True))


@artifact.command("resolve")
@click.argument("ref")
@click.option("--json", "as_json", is_flag=True)
def artifact_resolve(ref, as_json):
    """Resolve an alias to its immutable version."""
    from .artifacts import ArtifactRegistry

    resolved = _artifact_call(lambda: ArtifactRegistry().resolve(ref))
    if as_json:
        click.echo(json.dumps({"requested_ref": ref, "resolved_ref": str(resolved)}, sort_keys=True))
    else:
...
@click.option("--full", is_flag=True, help="Run the adapter's exhaustive verification.")
@click.option("--json", "as_json", is_flag=True)
def artifact_publish(manifest_path, verify, full, as_json):
    """Validate and atomically publish a manifest JSON file."""
    from .artifacts import ArtifactManifest, ArtifactRegistry

    manifest = _artifact_call(lambda: ArtifactManifest.from_json(manifest_path.read_text()))
    published = _artifact_call(
        lambda: ArtifactRegistry().publish(manifest, verify=verify, full=full)
    )
    click.echo(
        json.dumps(published.to_dict(), indent=2, sort_keys=True)
        if as_json else str(published.ref)
    )


@artifact.group("alias")
def artifact_alias():
    """Manage mutable aliases that point at immutable versions."""


@artifact_alias.command("set")
@click.argument("target_ref")
@click.argument("alias")
@click.option("--expected-previous", default=None)
@click.option("--json", "as_json", is_flag=True)
def artifact_alias_set(target_ref, alias, expected_previous, as_json):
    """Create an alias or update it with an optimistic precondition."""
    from .artifacts import ArtifactRegistry

    registry = ArtifactRegistry()
    alias_ref = _artifact_call(
        lambda: registry.set_alias(
            target_ref,
            alias,
            expected_previous=expected_previous,
        )
    )
    resolved = registry.resolve(alias_ref)
    if as_json:
        click.echo(json.dumps({"alias_ref": str(alias_ref), "resolved_ref": str(resolved)}, sort_keys=True))
    else:
        click.echo(f"{alias_ref} -> {resolved}")
...
@click.option("--full", is_flag=True)
@click.option("--json", "as_json", is_flag=True)
def artifact_verify(ref, full, as_json):
    """Re-run generic and type-specific verification."""
    from dataclasses import asdict
    from .artifacts import ArtifactRegistry

    report = _artifact_call(lambda: ArtifactRegistry().verify(ref, full=full))
    if as_json:
        click.echo(json.dumps(asdict(report), indent=2, sort_keys=True))
    else:
        click.echo(f"{'PASSED' if report.passed else 'FAILED'} ({report.adapter})")
        for issue in report.issues:
            click.echo(f"- {issue}")
        if report.summary:
            click.echo(json.dumps(report.summary, indent=2, sort_keys=True))
...
@click.argument("ref")
@click.option("--json", "as_json", is_flag=True)
def artifact_lineage(ref, as_json):
    ...
    value = {
        "ref": str(manifest.ref),
        "producer": asdict(manifest.producer),
    ...
    if as_json:
        click.echo(json.dumps(value, indent=2, sort_keys=True))
        return
    click.echo(f"Artifact: {value['ref']}")
    click.echo(f"Run:      {manifest.producer.run_id or '-'}")
    click.echo(f"Jobs:     {', '.join(manifest.producer.job_ids) or '-'}")
    click.echo(f"Source:   {manifest.producer.repo or '-'}@{manifest.producer.commit or '-'}")
    click.echo(f"Inputs:   {', '.join(value['dependencies']) or '-'}")
